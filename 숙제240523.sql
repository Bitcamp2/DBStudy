...
--	 SELECT MONTHS_BETWEEN(TRUNC(SYSDATE, 'DD'), TO_DATE('20230523', 'YYYYMMDD'))
	 , ROUND(MONTHS_BETWEEN(SYSDATE, HDATE) / 12) || '년' 
	 || ROUND(MONTHS_BETWEEN(SYSDATE, HDATE) - ROUND(MONTHS_BETWEEN(SYSDATE, HDATE) / 12) * 12) || '개월' 
	 || ROUND((MONTHS_BETWEEN(SYSDATE, HDATE) - ROUND(MONTHS_BETWEEN(SYSDATE, HDATE) - ROUND(MONTHS_BETWEEN(SYSDATE, HDATE) / 12) * 12)) / 24) 
	 || '일' AS 오늘까지근무기간
	 FROM EMP;
